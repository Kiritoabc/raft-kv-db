...
func (rf *Raft) GetState() (term int, isLeader bool) {
	rf.mu.Lock()
	defer rf.mu.Unlock()
	log.Log.Infof("%v: 获取 %v 的状态", rf.currentTerm, rf.me)
	term = rf.currentTerm
...
func (rf *Raft) persist() {
	...
	func() {
		...
		log.Log.Infof("%v: %v 持久化了 votedFor=%v, logs 的最后一个是 %v",
...
func (rf *Raft) readPersist(data []byte) {
	...
	if err1 != nil || err2 != nil || err3 != nil {
		log.Log.Fatalf("%v 恢复失败 err1=%v, err2=%v, err3=%v", rf.me, err1, err2, err3)
	} else {
		...
		log.Log.Debugf("%v 恢复了 votedFor=%v, term=%v, logs=%v", rf.me, votedFor, currentTerm, logs)
	}
}
...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) error {
	...
	if args.Term < rf.currentTerm {
		log.Log.Infof("%v: %v 认为 %v 的投票请求 {term=%v, lastIdx=%v, lastTerm=%v} 过时了",
	...
	if args.Term > rf.currentTerm {
		log.Log.Infof("%v: %v 接收到投票请求后发现自己过时，变回追随者，新的任期为 %v", rf.currentTerm, rf.me, args.Term)
		// 当前服务器更新自己的任期号
	...
	go rf.persist()
	log.Log.Infof("%v: %v 对 %v 的投票 {term=%v, lastIdx=%v, lastTerm=%v} 结果为 %v, votedFor=%v",
...
func (rf *Raft) AppendEntries(args *AppendEntriesArgs, reply *AppendEntriesReply) error {
	...
	if args.Term > rf.currentTerm {
		log.Log.Infof("%v: %v 收到条目增加请求后发现自己过时，变回追随者，新任期为 %v", rf.currentTerm, rf.me, args.Term)
		rf.currentTerm = args.Term
	...
	if args.Term < rf.currentTerm {
		if len(args.Entries) > 0 {
			log.Log.Infof("%v: %v 接收到 %v 的复制请求 {{%v ~ %v} term=%v leaderCommit=%v}，结果为：已过时",
				rf.currentTerm, rf.me, args.LeaderId, args.Entries[0], args.Entries[len(args.Entries)-1],
				args.Term, args.LeaderCommit)
		} else {
			log.Log.Infof("%v: %v 接收到 %v 的复制请求 {term=%v leaderCommit=%v}，结果为：已过时",
	...
	if args.PrevLogIndex >= lenLogsCurrent || rf.logs[args.PrevLogIndex].Term != args.PrevLogTerm {
		if len(args.Entries) > 0 {
			log.Log.Infof("%v: %v 接收到 %v 的复制请求 {{%v ~ %v} term=%v leaderCommit=%v}，结果为：不匹配",
				rf.currentTerm, rf.me, args.LeaderId, args.Entries[0], args.Entries[len(args.Entries)-1],
				args.Term, args.LeaderCommit)
		} else {
			log.Log.Infof("%v: %v 接收到 %v 的复制请求 {term=%v leaderCommit=%v}，结果为：不匹配",
				rf.currentTerm, rf.me, args.LeaderId, args.Term, args.LeaderCommit)
		}
	...
	go rf.persist()
	if len(args.Entries) > 0 {
		log.Log.Infof("%v: %v 接收到 %v 的复制请求 {{%v ~ %v} term=%v leaderCommit=%v}，结果为：成功",
			rf.currentTerm, rf.me, args.LeaderId, args.Entries[0], args.Entries[len(args.Entries)-1],
			args.Term, args.LeaderCommit)
	} else {
		log.Log.Infof("%v: %v 接收到 %v 的复制请求 {term=%v leaderCommit=%v}，结果为：成功",
			rf.currentTerm, rf.me, args.LeaderId, args.Term, args.LeaderCommit)
	}
	log.Log.Infof("%v: %v 接收到 %v 的增加条目(%v)请求，结果为 %v",
		rf.currentTerm, rf.me, args.LeaderId, *args, reply.Success)
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	if isLeader {
		...
		log.Log.Infof("-----%v 是领导，增加了新的条目为 {%v %v %v}", rf.me, command, term, index)
	}
...
func (rf *Raft) Kill() {
	...
	log.Log.Infof("----- %v 结束 -----", rf.me)
	func() {
		rf.mu.Lock()
		defer rf.mu.Unlock()
		log.Log.Infof("%v: %v 的日志内容：%v", rf.currentTerm, rf.me, rf.logs)
	}()
}
...
func (rf *Raft) sendLogEntry(flag int) {
	...
	loglen := len(rf.logs)
	//if flag == CopyEntries {
	log.Log.Infof("%v: %v 开始复制，最后一个条目为 %v，最后提交的索引为 %v",
	...
	for finish := 1; finish < rf.peersLen; finish++ {
		...
		if result == -1 {
			...
			if grantedCount > tgt {
				func() {
					...
					if rf.identity == LEADER && commit < len(rf.logs) && commit > rf.commitIndex {
						...
						go rf.apply()
						//if flag == CopyEntries {
						log.Log.Infof("%v: %v 提交成功，提交的最大的索引为 %v，最后复制过去的是 %v",
			...
			if notGrantedCount > tgt {
				//if flag == CopyEntries {
				log.Log.Infof("%v: %v 提交失败，准备提交的索引为 %v，退回追随者", argsTemplate.Term, rf.me, loglen-1)
				//}
				func() {
					rf.mu.Lock()
					defer rf.mu.Unlock()
			...
			func() {
				...
				if rf.currentTerm < result {
					...
					rf.votedFor = -1
					rf.identity = FOLLOWER
					log.Log.Infof("%v: %v 收到条目增加响应后发现自己过时，变成追随者，新任期为 %v",
...
func (rf *Raft) apply() {
	...
	if rf.commitIndex < rf.lastApplied {
		log.Log.Fatalf("%v: %v 调用 apply()： commitIndex(%v) < lastApplied(%v)",
	...
	log.Log.Infof("%v: %v 最后应用的是 %v, logs 里最后一个是 %v",
			...
			if grantedCount > tgt {
				func() {
					...
					if rf.identity != LEADER {
						...
						log.Log.Infof("%v: %v 赢得了选举，变为领导者，发送了初始心跳",
							args.Term, rf.me)
					}
				}()
				// 选举成功标志
			...
			if args.Term < reply.Term {
				func() {
					...
					if rf.currentTerm < reply.Term {
						log.Log.Infof("%v: %v 发送投票后发现自己过时，变回追随者", args.Term, rf.me)
